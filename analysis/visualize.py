...
if not csv_path.exists():
    print("[INFO] cr39_output.csv not found — generating synthetic data for demo.")
    rng = np.random.default_rng(42)
    N = 50_000
    hit_mask = rng.random(N) > 0.02          # ~98 % hit rate at 2.5 MeV / 3 cm air
    edep   = rng.normal(2.35, 0.08, N) * hit_mask   # MeV — most protons stop inside
    tlen   = rng.normal(0.47, 0.02, N) * hit_mask   # mm  — range in CR-39
    let    = np.where(tlen > 0, edep / tlen, 0.)
    ex     = rng.normal(0, 1.0, N)           # mm Gaussian beam profile
    ey     = rng.normal(0, 1.0, N)
    df = pd.DataFrame(dict(EventID=np.arange(N), Edep_MeV=edep,
                           TrackLen_mm=tlen, LET_MeV_mm=let,
                           EntryX_mm=ex, EntryY_mm=ey, Hit=hit_mask.astype(int)))
else:
    df = pd.read_csv(csv_path)

hits = df[df["Hit"] == 1].copy()
print(f"Total events : {len(df):,}")
print(f"Hits in CR-39: {len(hits):,}  ({100*len(hits)/len(df):.1f} %)")
print(f"Mean Edep    : {hits['Edep_MeV'].mean():.3f} ± {hits['Edep_MeV'].std():.3f} MeV")
print(f"Mean LET     : {hits['LET_MeV_mm'].mean():.3f} MeV/mm")

# ── Figure layout ─────────────────────────────────────────────────────────────
fig = plt.figure(figsize=(14, 10))
gs  = GridSpec(2, 3, figure=fig, hspace=0.38, wspace=0.38)

ax1 = fig.add_subplot(gs[0, 0])   # Energy deposition spectrum
ax2 = fig.add_subplot(gs[0, 1])   # LET distribution
ax3 = fig.add_subplot(gs[0, 2])   # Track length distribution
ax4 = fig.add_subplot(gs[1, 0])   # Beam spot (2D histogram)
ax5 = fig.add_subplot(gs[1, 1])   # Radial fluence profile
ax6 = fig.add_subplot(gs[1, 2])   # LET vs Edep scatter + KDE

# ── Panel 1: Energy deposition spectrum ───────────────────────────────────────
...
ax1.set_ylabel("Probability density (MeV$^{-1}$)")
ax1.set_title("Energy Deposition Spectrum")
ax1.legend(framealpha=0.9)

# Theoretical Bragg peak label
ax1.text(0.62, 0.82, "Bragg peak\nregion", transform=ax1.transAxes,
         fontsize=9, color="gray", ha="center")

# ── Panel 2: LET distribution ──────────────────────────────────────────────────
...
ax2.set_ylabel("Probability density (mm MeV$^{-1}$)")
ax2.set_title("Linear Energy Transfer in CR-39")
ax2.legend(framealpha=0.9)
# Water-equivalent LET reference (protons 2.5 MeV ~ 50 keV/µm ≡ 50 MeV/mm in water)
ax2.text(0.97, 0.60, r"Proton range in CR-39" "\n" r"$R \approx 0.1$ mm",
         transform=ax2.transAxes, ha="right", fontsize=8.5, color="dimgray",
...
rect = plt.Rectangle((-15, -15), 30, 30, lw=1.5, ec="cyan", fc="none", ls="--")
ax4.add_patch(rect)
ax4.set_xlim(-lim, lim)
ax4.set_ylim(-lim, lim)

# ── Panel 5: Radial fluence profile ───────────────────────────────────────────
r = np.sqrt(hits["EntryX_mm"]**2 + hits["EntryY_mm"]**2)
r_bins = np.linspace(0, lim, 60)
r_mid  = 0.5*(r_bins[:-1] + r_bins[1:])
dr     = np.diff(r_bins)
counts_r, _ = np.histogram(r, bins=r_bins)
area_annulus = np.pi * ((r_bins[1:])**2 - (r_bins[:-1])**2)   # mm²
fluence = counts_r / area_annulus   # protons/mm²  (normalised to counts)
...
ax5.set_ylabel("Fluence (rel. protons mm$^{-2}$)")
ax5.set_title("Radial Fluence Profile")
ax5.legend(framealpha=0.9)

# ── Panel 6: LET vs Edep 2D KDE contours ─────────────────────────────────────
# Subsample for speed
...
ax6.hexbin(x6, y6, gridsize=50, cmap="YlOrRd",
...
sm.set_array([])
cb6 = fig.colorbar(sm, ax=ax6, pad=0.02)
cb6.set_label("Counts / hexbin")
...
print(f"\nFigure saved: {out}  +  {out.with_suffix('.png')}")
plt.show()
